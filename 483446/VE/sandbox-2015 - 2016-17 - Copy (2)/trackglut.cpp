...
double cubefacesize = 0.2; //// *** first stage -  choose a value to fit in your window;
...
double screenup = (screenheight / 2.0) + 1.75;     //// *** second stage -  choose a positive value for ground-to-screen-bottom-edge distance along OpenGl +y according to the spec
...
void OnDisplay() {
	...
	for (int i = 0; i < numeyes; i++) {
		...
		q_vec_set(scale, cubefacesize, cubefacesize, cubefacesize);
		scaleObject(tmp3, scale);

		q_vec_set(translation, ((cubefacesize / 2.0) + screendist), 0.0, screenup);  //what is the significance of the halved translation in the original?
