...
double cubefacesize = 1.0; //// *** first stage -  choose a value to fit in your window;
double screendist = 0.0;   //// *** second stage -  choose a positive value for origin-to-screen distance along OpenGl -z according to the spec
double screenup = 0.0;     //// *** second stage -  choose a positive value for ground-to-screen-bottom-edge distance along OpenGl +y according to the spec
...
void ViewSystem(const bool righteye) {
	...
	if (bInperson) {   // first-person moving view - stages three and four
		...
		//AsymmetricFrustum(proj, left, right, bottom, top, 0.1, 10.0); //numerical values for left, right etc to make look similar to the original
		...
		//double unsignedleft = ; // calculation here using eye position Pos, screen height and viewaspect
		//double unsignedright = ; // calculation here using eye position Pos, screen height and viewaspect
		//double unsigned bottom = ; // calculation here using eye position Pos and screenup
		//double unsignedtop = ; // calculation here using eye position Pos, screen height and screenup
		//double S = ; // calculation here using eye position Pos, near (= 0.1) and screendist
		//AsymmetricFrustum(proj, left, right, bottom, top, 0.1, 10.0); // uses above, scaled AND with appropriate l, r, t, b signs practised at stage 3
	}
	else { // static 
	...
	if (bInfront) {
		...
		q_vec_set(eyepos, 0.0, 2.0, 4.0);  //// *** fourth (+subsequent) stage(s) - the TRANSFORMED eye position Pos (NB this is OpenGl space)
		q_vec_set(focus, 0.0, 0.0, 0.0); //// *** fourth (+subsequent) stage(s) -  the TRANSFORMED looked-at position opposite Pos (NB this is OpenGl space)
		q_vec_set(up, 0.0, 1.0, 0.0);
	}
	else if (bSideon) { // not impemented yet, so make same as infront
		q_vec_set(eyepos, 0.0, 2.0, 4.0);  //// *** second stage - same plane as screen at around glasses height, right-hand edge of the ground plane (NB this is OpenGl space)
		q_vec_set(focus, 0.0, 0.0, 0.0); //// *** second stage - opposite side of scene to eye (reflect in gl's x=0 plane)
		q_vec_set(up, 0.0, 1.0, 0.0);
	}
...
void OnDisplay() {
	...
	for (int i = 0; i < numeyes; i++) {
		...
		q_vec_set(translation, 0.5, (((viewaspect * screenheight) / 2.0) - 1.5), (screenheight / 2.0));  //what is the significance of the halved translation in the original?
		...
		q_vec_set(scale, ((cubefacesize * cubefacesize) * 0.2), ((cubefacesize * cubefacesize) * 0.2), ((cubefacesize * cubefacesize) * 0.2));
		scaleObject(tmp3, scale);

		q_vec_set(translation, 1.5, 1.5, (((cubefacesize * cubefacesize) * 0.2) / 2.0));  //what is the significance of the halved translation in the original?
		...
		q_vec_set(axis, 0.0, 0.0, 1.0);
		rotateObject(tmp3, axis, -(((cubefacesize - 1.0) * 100) + 45.0));
...
void OnKeyboard(unsigned char key, int x, int y) {
	...
	case '+':  // inflate the cube and monitor height
		cubefacesize += 0.01;  // 1cm increments
		screenheight += 0.01;
		break;
	case '-':  // deflate the cube and monitor height
		cubefacesize -= 0.01;  // 1cm decrements
		screenheight -= 0.01;
		break;
